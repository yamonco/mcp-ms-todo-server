...
Usage:
  ADMIN_API_KEY=test-key TOOL_SCHEMA_DIR=./app/tools python smoke_test.py
"""
import os
import os.path
...
def main():
    ...
    # 2) health with key
    r = client.get("/health", headers={"x-api-key": os.environ["ADMIN_API_KEY"]})
    must(r.status_code == 200, f"/health expected 200, got {r.status_code}")
    j = r.json()
    must(j.get("status") == "ok", "health status not ok")

    # 3) create a user API key (template=lite)
    r = client.post(
        "/admin/api-keys",
        headers={"x-api-key": os.environ["ADMIN_API_KEY"]},
        json={"template": "lite"},
    )
    must(r.status_code == 200, f"/admin/api-keys expected 200, got {r.status_code}")
    user_key = r.json().get("api_key")
    must(bool(user_key), "failed to create user api key")

    # 4) manifest with user key
    r = client.get("/mcp/manifest", headers={"x-api-key": user_key})
    ...
    # 5) JSON-RPC initialize (user key)
    payload = {"jsonrpc": "2.0", "id": 1, "method": "initialize"}
    r = client.post("/mcp", headers={"x-api-key": user_key}, json=payload)
    ...
    # 7) tools/call invalid params → -32602 (user key)
