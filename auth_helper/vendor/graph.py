...
except Exception:  # pragma: no cover - allow script-style imports
    from config import Settings  # type: ignore

def get_user_device_code_token(tenant: str, client_id: str, scopes: str = "offline_access Tasks.ReadWrite") -> dict:
    """유저용 device code flow: 인증 URL/코드 출력, 인증 완료 시 access/refresh token 반환"""
    t = (tenant or "").strip() or "organizations"
    device_code_url = f"https://login.microsoftonline.com/{t}/oauth2/v2.0/devicecode"
    token_url = f"https://login.microsoftonline.com/{t}/oauth2/v2.0/token"
    resp = requests.post(device_code_url, data={"client_id": client_id, "scope": scopes}, timeout=10)
    if resp.status_code != 200:
        raise RuntimeError(f"Device code request failed: {resp.status_code} {resp.text[:200]} (url={device_code_url})")
    dc = resp.json()
    print(f"[인증 안내] {dc.get('message', '')}")
    print(f"[인증 안내] Tenant: '{t}' (device_code_url={device_code_url})")
    ...
    while True:
        ...
        if poll.status_code == 200:
            tok = poll.json()
            print("[인증 성공] access_token/refresh_token 발급 완료")
            return tok
...
def get_admin_access_token(cfg: Settings, *, interactive: bool = False) -> str:
    ...
    # Guard tenant value; provide sensible defaults on failure
    t = (tenant or "").strip() or "organizations"
    device_code_url = f"https://login.microsoftonline.com/{t}/oauth2/v2.0/devicecode"
    token_url = f"https://login.microsoftonline.com/{t}/oauth2/v2.0/token"
    public_client_id = "04b07795-8ddb-461a-bbee-02f9e1bf7b46"  # Microsoft official public client
    scope = "https://graph.microsoft.com/.default offline_access"
    # Try device code request, with fallback tenants if 404/400
    resp = requests.post(device_code_url, data={"client_id": public_client_id, "scope": scope}, timeout=10)
    if resp.status_code != 200:
        details = f"Device code request failed: {resp.status_code} {resp.text[:200]} (url={device_code_url})"
        # fallback: try organizations/common when tenant is suspicious
        for fallback in ("organizations", "common"):
            if fallback in device_code_url:
                continue
            try_url = f"https://login.microsoftonline.com/{fallback}/oauth2/v2.0/devicecode"
            resp2 = requests.post(try_url, data={"client_id": public_client_id, "scope": scope}, timeout=10)
            if resp2.status_code == 200:
                print(f"[INFO] Falling back to tenant='{fallback}' for device code flow")
                resp = resp2
                token_url = f"https://login.microsoftonline.com/{fallback}/oauth2/v2.0/token"
                break
        if resp.status_code != 200:
            raise RuntimeError(details)
    dc = resp.json()
    print(f"[Sign-in] {dc.get('message', '')}")
    print(f"[Sign-in] Tenant used: '{t}' (device_code_url={device_code_url})")
    interval = int(dc.get("interval", 5))
    expires = int(dc.get("expires_in", 900))
    start = time.time()
    while True:
